    ...
    if (win_i >= 0 && e->detail == Above && (e->value_mask & CWStackMode)) {
        if (e->value_mask & CWSibling) {
            ...
            if (parent) {
                ...
                // explicitly request property cache as composite window might
                // not exist yet
                if (MWindowPropertyCache *pc = getPropertyCache(e->window)) {
            ...
            if (i->isMapped()
                ...
                for (wi = stacking_list.size() - 1; wi >= 0; --wi) {
                    ...
                    if (cw && cw->isMapped() && (cw->isAppWindow(true)
                        ...
                        // stack desktop accordingly to keep
                        // MCompositeWindow::behind() working
                        lower_i = wi;
                    }
                }
